    ...
    def log_progress(self, force=False):
        ...
        with self.lock:
            if force or current_time - self.last_log_time >= 5:
                ...
                pct = self.completed * 100 // self.total_targets if self.total_targets > 0 else 0
    ...
    try:
        ...
        if os.path.exists(filename):
            ...
            if file_size == 0 and len(results) > 0:
                print("⚠️ 警告: 文件为空但结果不为空，尝试重新保存...")
                # 重试一次
                with open(filename, 'w', encoding='utf-8') as f2:
                    for r in results:
                        f2.write(r + '\n')
                print(f"✅ 重新保存完成，文件大小: {os.path.getsize(filename)} 字节")
    except Exception as e:
        print(f"❌ 保存结果失败: {e}")
        # 备用保存方式
        try:
...
def main():
    ...
    if total_targets > 0 and scan_elapsed > 0:
    ...
    try:
        with open(stats_file, 'w') as f:
            json.dump({
                ...
                'speed': total_targets/scan_elapsed if scan_elapsed > 0 else 0,
                'results': results
            }, f, indent=2)
        print(f"📊 统计报告已保存: {stats_file}")
    except Exception as e:
        print(f"⚠️ 保存统计报告失败: {e}")
